        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!File.Exists(path))
            {
                using (StreamWriter sw = File.CreateText(path+"\\file.txt"))
                {
                    ...
                    foreach (DirectoryInfo df in di.GetDirectories("*", SearchOption.AllDirectories))
                    {
                        sw.WriteLine(df);
                        sw.WriteLine("->");
                    ...
                    sw.WriteLine("Список каталогов");
                    foreach (var item in info.GetDirectories())
                    {
                        sw.WriteLine(item.Name);
                        sw.WriteLine("Список файлов каталога");
                        foreach (var itf in item.GetFiles())
                        {
                            sw.WriteLine(itf.Name);
                        }
                        sw.WriteLine("Список подкаталогов");
                        foreach(var it in item.GetDirectories())
                        {
                            foreach(var itd in it.GetDirectories())
                            {
                                sw.WriteLine(itd.Name);
                            }
                            sw.WriteLine(it.Name);
                            sw.WriteLine("Список файлов подкаталога");
                            foreach(var itf in it.GetFiles())
                            {
                                sw.WriteLine(itf.Name);
                            }
                        }
